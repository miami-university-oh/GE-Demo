...
def mouse_callback(event, x, y, flags, param):
    """Handle left-click on the live view."""
...
def compute_camera_height():
    """Average Z (depth) across all clicked floor points."""
    ...
    if std > 0.02:
        print(f"    ⚠  High variance — floor may not be flat, or points are on objects.")
    print(f"    → Camera is ~{camera_height:.3f} m above the floor.\n")


def compute_matrix():
    """
    Build T_cam_floor for a straight-down ceiling mount.

    For a camera mounted on the ceiling looking straight down:
      - Camera Z axis points toward the floor (positive Z = downward).
      - Floor X ≈ Camera X
      - Floor Y ≈ Camera Y
      - Floor Z = 0 (floor plane)

    The transform is:
        p_floor = T_cam_floor @ p_cam

    With the camera at height H pointing straight down:
        T_cam_floor = [[ 1,  0,  0,   tx ],
                       [ 0,  1,  0,   ty ],
                       [ 0,  0, -1,   H  ],
                       [ 0,  0,  0,   1  ]]

    tx, ty are the camera's lateral offset from the floor-frame origin.
    If you define the floor origin as "directly below the camera", tx=ty=0.
    """
    global camera_height
    ...
    for i, (u, v, xc, yc, zc, d) in enumerate(clicked_points):
        ...
        label = " ← ROBOT BASE" if robot_base_point and \
                np.allclose([xc, yc, zc], robot_base_point, atol=0.001) else ""
    ...
    if robot_base_point is not None:
        ...
        print(f"  ⚠  Robot base not marked. Click the UR5e base, press 'r', then 'm' again.")
...
def main():
    ...
    try:
        while True:
            ...
            for i, (u, v, xc, yc, zc, d) in enumerate(clicked_points):
                is_robot = robot_base_point is not None and \
                           np.allclose([xc, yc, zc], robot_base_point, atol=0.001)
                color = (0, 0, 255) if is_robot else (0, 255, 0)
                cv2.circle(display, (u, v), 8, color, 2)
                label = f"#{i} R" if is_robot else f"#{i}"
                cv2.putText(display, f"{label} d={d:.2f}m",
                            (u + 12, v - 5), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5, color, 1)

            # Show depth at cursor (top-left HUD).
            cv2.putText(display, f"Points: {len(clicked_points)}  |  "
                        f"Height: {camera_height:.3f}m" if camera_height else
                        f"Points: {len(clicked_points)}  |  Height: (press 'h')",
    ...
    if robot_base_point:
        print(f"  Robot base (cam): ({robot_base_point[0]:.4f}, "
