...
for idx, name in model.names.items():
    if name.lower().replace(" ", "-") in ALLOWED_CLASSES or name.lower() in ALLOWED_CLASSES:
        lname = name.lower()
...
def capture_and_infer():
    ...
    while running:
        ...
        cv2.putText(img, banner, (10, 19),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.52, banner_color, 1, cv2.LINE_AA)

        # ── Timestamp ──
        ts = datetime.now().strftime("%H:%M:%S")
        cv2.putText(img, f"CAM-02 · Amcrest · YOLOv8 best.pt · {ts}",
                    (12, h - 10), cv2.FONT_HERSHEY_SIMPLEX,
                    0.36, (0, 220, 180), 1, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--camera", default=CAMERA_URL,
                        help="Camera URL (RTSP or HTTP stream)")
